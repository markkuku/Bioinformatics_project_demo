        ...
        for i in range(1,len(self.seqs[index_x])+1):
            ...
            #determine who is the maximal value
                ...
                if (float(self.alignment_array[i-1][j-1])+ matched_score > temp_max):
                    ...
                    temp_dir = "ul"
                if (float(self.alignment_array_up[i-1][j-1])+matched_score > temp_max):
                    temp_max = float(self.alignment_array_up[i-1][j-1])+matched_score
                    temp_dir = "up"
                if (float(self.alignment_array_left[i-1][j-1])+matched_score > temp_max):
                    temp_max = float(self.alignment_array_left[i-1][j-1])+matched_score
                    temp_dir = "left"                     
