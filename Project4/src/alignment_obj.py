...
class alignment_obj:
    ...
    def get_align_aff(self,index_x,index_y,gap_op,gap_exd,semi_opt,time_delay,win,textfield,print_opt):
        ...
        for i in range(1,len(self.seqs[index_x])+1):
            ...
            if (semi_opt == 0):
                ...
                self.alignment_array_up[i][0] =  -999999
        ...
        for j in range(1,len(self.seqs[index_y])+1):
            ...
            if (semi_opt == 0):
                ...
                self.alignment_array_left[0][j] = -999999
    ...
    def print_proc(self,index_x,index_y,x,y,opt,time_delay,win,textfield,open):
        ...
        opt_des = ['','Main recursion: \n S_main[i,j] = Max(S_Main[i-1,j-1]+BLO(Si,Sj), S_left[i,j], S_up[i,j])'
        ...
        if (opt == 1):
            print_get_max[0] = self.alignment_array_M[index_x][index_y]
            print_get_max[1] = self.alignment_array_up[index_x][index_y]
            print_get_max[2] = self.alignment_array_left[index_x][index_y]
            text_tmp += "Current: Max ( %s, %s, %s ) Direction: (upper left, upper, left)\n" % (print_get_max[0],print_get_max[1],print_get_max[2]) 
        elif (opt == 2):
            print_get_max[0] = self.alignment_array_M[index_x-1][index_y]-self.gap_open
            ...
            print_get_max[0] = self.alignment_array_M[index_x][index_y-1]-self.gap_open
